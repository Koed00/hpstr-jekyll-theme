...
A new version of [pip-tools](https://github.com/nvie/pip-tools) was recently released and it brings with it a much cleaner work-flow to manage your virtual environments for Python.
...
First we create a `requirements.in` file which will list all the basic package requirements for our projects. I will not insult you by explaining how to create and edit a file, but take care you use the correct extension `.in` instead of `.ini` or `.txt`. Here I'm just putting in some standard packages I would use for most Django projects:
...
As you can see `pip-compile` neatly pinned all the packages and their own requirements.Now you know exactly why e.g. `sqlparse` is installed or `six` and by which package. If you wonder why you should be pinning your requirements like this, you can read about it in this [article](http://nvie.com/posts/better-package-management/) by the pip-tools author himself.
...
{% endhighlight %}
Most software will only look for a `requirements.txt` file to install dependencies

## Pip sync
Until now we haven't installed anything yet in our virtualenv. We could simpy do `$ pip install -r requirements.txt` but what if there are already packages previously?. The new `pip-sync` command takes care of this. It not only installs your requirements, it also removes anything that is not in your requirement files. You'll soon find out if you forgot to list any requirements in the `.in` files.
Let's run:
{% highlight bash %}
...
Now let's assume I've decided that `django-debug-toolbar` is a great package but I don't want to use it on my servers. Normally you would run `$ pip uninstall djang-debug-toolbar` and remove the entry manually it from your 'requirements.txt'. Unfortunately this doesn't remove the `sqlparse` package, unless you remembered, and months later you'll be wondering why it is in your requirements file.
...
As you can see both the toolbar and its sqlparse dependency have been removed.
